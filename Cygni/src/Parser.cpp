...
std::shared_ptr<SourceDocument> Parser::ParseProgram() {
  std::shared_ptr<SourceDocument> program =
      std::make_shared<SourceDocument>(document);
  ...
  while (!IsEof()) {
    if (Look().tag == Tag::Class) {
      ...
      throw ParserException(document, Look().line, Look().column,
                            U"expecting 'class', 'module' or 'interface'");
    }
  }
  return program;
}
...
ExpPtr Parser::ParseOr() {
  ...
  while (Look().tag == Tag::Or) {
    Match(Tag::Or);
    auto y = ParseAnd();
    x = std::make_shared<BinaryExpression>(Pos(start), ExpressionType::Or, x,
                                           y);
  }
  return x;
}
...
ExpPtr Parser::ParseAnd() {
  ...
  while (Look().tag == Tag::And) {
    ...
    x = std::make_shared<BinaryExpression>(Pos(start), ExpressionType::And, x,
                                           y);
  }
  return x;
}
...
ExpPtr Parser::ParseEquality() {
  ...
  while (Look().tag == Tag::Equal || Look().tag == Tag::NotEqual) {
    ...
    if (t.tag == Tag::Equal) {
      x = std::make_shared<BinaryExpression>(Pos(start), ExpressionType::Equal,
                                             x, y);
    } else {
...
ExpPtr Parser::ParseExpr() {
  ...
  while (Look().tag == Tag::Add || Look().tag == Tag::Subtract) {
    ...
    auto y = ParseTerm();
    if (t.tag == Tag::Add) {
      x = std::make_shared<BinaryExpression>(Pos(start), ExpressionType::Add, x,
                                             y);
    } else {
      x = std::make_shared<BinaryExpression>(Pos(start),
...
ExpPtr Parser::ParseTerm() {
  ...
  while (Look().tag == Tag::Multiply || Look().tag == Tag::Divide) {
    ...
    if (t.tag == Tag::Multiply) {
      ...
      x = std::make_shared<BinaryExpression>(Pos(start), ExpressionType::Divide,
                                             x, y);
    }
  }
  return x;
...
ExpPtr Parser::ParseFactor() {
  if (Look().tag == Tag::LeftParenthesis) {
    ...
    const Token &start = Look();
    Advance();
    return std::make_shared<ParameterExpression>(Pos(start), name,
                                                 Type::Unknown());
  } else if (Look().tag == Tag::New) {
    return ParseNewExpression();
  } else {
    throw ParserException(
        document, Look().line, Look().column,
...
ExpPtr Parser::IfStatement() {
  ...
  if (Look().tag == Tag::Else) {
    ...
    auto empty = std::make_shared<DefaultExpression>(Pos(Look()), Type::Void());
    return std::make_shared<ConditionalExpression>(Pos(start), condition,
                                                   ifTrue, empty);
  }
}
...
ExpPtr Parser::ParseVar() {
  ...
  if (Look().tag == Tag::Colon) {
    ...
    Match(Tag::Assign);
    auto value = ParseOr();
    auto variable = std::make_shared<ParameterExpression>(Pos(Look()), name,
                                                          Type::Unknown());
    return std::make_shared<VarDefExpression>(Pos(start), variable,
                                              Type::Unknown(), value);
  }
}

std::shared_ptr<FieldInfo>
Parser::ParseFieldDefinition(AccessModifier modifier,
  ...
  if (Look().tag == Tag::Assign) {
    Match(Tag::Assign);
    auto value = ParseOr();
    return std::make_shared<FieldInfo>(Pos(start), modifier, isStatic,
...
Parser::ParseMethodDefinition(AccessModifier modifier,
                              Table<std::u32string, AnnotationInfo> annotations,
  ...
  if (Look().tag == Tag::LeftBrace) {
    ...
    return std::make_shared<MethodInfo>(Pos(start), modifier, isStatic,
                                        selfType, annotations, name, parameters,
                                        returnType, body);
  } else {
    auto empty = std::make_shared<DefaultExpression>(Pos(Look()), returnType);
    return std::make_shared<MethodInfo>(Pos(start), modifier, isStatic,
                                        selfType, annotations, name, parameters,
                                        returnType, empty);
  }
...
std::shared_ptr<ArrayType> Parser::ParseArrayType() {
  ...
  if (args.size() == 1) {
    ...
    throw ParserException(document, Look().line, Look().column,
                          Format(U"wrong number of type arguments for the "
                                 U"'Array' type. should be 1 instead of {}.",
...
std::shared_ptr<ClassInfo> Parser::ParseDefClass(const Token &start,
  ...
  while (!IsEof() && Look().tag != Tag::RightBrace) {
    ...
    if (Look().tag == Tag::Var) {
      ...
      throw ParserException(
          document, Look().line, Look().column,
          Format(U"unexpected token type '{}', expecting 'var' or 'def'",
...
std::shared_ptr<InterfaceInfo> Parser::ParseDefInterface() {
  ...
  while (!IsEof() && Look().tag != Tag::RightBrace) {
    ...
    if (Look().tag == Tag::Def) {
      ...
      auto method = ParseMethodDefinition(AccessModifier::Public, annotations,
                                          false, selfType);
      info->methodDefs.Add(method->name,
                           method); // only definition, no implementation
    } else {
      throw ParserException(
          document, Look().line, Look().column,
          Format(U"unexpected token type '{}', expecting 'def'",
                 Enum<Tag>::ToString(Look().tag)));
    }
  }
  Match(Tag::RightBrace);
