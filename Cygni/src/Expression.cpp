...
MethodInfo::MethodInfo(
    SourcePosition position, AccessModifier modifier, bool isStatic,
    TypePtr selfType, Table<std::u32string, AnnotationInfo> annotations,
    std::u32string name,
    std::vector<std::shared_ptr<ParameterExpression>> parameters,
    TypePtr returnType, ExpPtr body)
    : position{position}, modifier{modifier}, isStatic{isStatic},
      selfType{selfType}, annotations{annotations}, name{name},
  ...
  this->signature = std::make_shared<FunctionType>(selfType, name,
                                                   parameterTypes, returnType);
}

ClassInfo::ClassInfo(SourcePosition position, PackageRoute route,
                     std::u32string name)
    : position{position}, route{route}, name{name} {}

ModuleInfo::ModuleInfo(SourcePosition position, PackageRoute route,
                       std::u32string name)
    : position{position}, route{route}, name{name} {}
...
SourceDocument::SourceDocument(std::shared_ptr<FileLocation> document)
    : fileLocation{document} {}

TypeAlias::TypeAlias(SourcePosition position, PackageRoute route,
                     std::u32string typeName, std::u32string alias)
    : position{position}, route{route}, typeName{typeName}, alias{alias} {}

PackageRouteStatement::PackageRouteStatement(SourcePosition position,
                                             PackageRoute route)
...
// 		{
// 			package->interfaceDefs.Add(interfaceInfo->name,
// interfaceInfo);
// 		}
...
Project::GetModule(std::shared_ptr<ModuleType> moduleType) {
...
Project::GetClass(std::shared_ptr<ClassType> classType) {
...
Project::GetInterface(std::shared_ptr<InterfaceType> interfaceType) {
...
Project::ResolveType(std::shared_ptr<UnresolvedType> unresolvedType) {
  ...
  if (packages.count(route)) {
    ...
    if (pkg->classes.count(name)) {
      ...
      return {std::make_shared<InterfaceType>(interfaceInfo->route,
                                              interfaceInfo->name)};
    } else {
      return {};
    }
