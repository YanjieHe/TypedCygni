	...
	while (Look().tag == Tag::Equal || Look().tag == Tag::NotEqual) {
		...
		if (t.tag == Tag::Equal) {
			...
			x = std::make_shared<BinaryExpression>(
				GetLoc(start), ExpressionType::NotEqual, x, y);
		}
	}
	return x;
}
...
ExpPtr Parser::ParseRelation() {
	...
	if (Look().tag == Tag::GreaterThan || Look().tag == Tag::LessThan ||
		...
		if (t.tag == Tag::GreaterThan) {
			...
			return std::make_shared<BinaryExpression>(
				GetLoc(start), ExpressionType::LessThan, x, y);
...
ExpPtr Parser::ParseExpr() {
	...
	while (Look().tag == Tag::Add || Look().tag == Tag::Subtract) {
		...
		if (t.tag == Tag::Add) {
			...
			x = std::make_shared<BinaryExpression>(
				GetLoc(start), ExpressionType::Subtract, x, y);
		}
	}
	return x;
}
...
ExpPtr Parser::ParseTerm() {
	...
	while (Look().tag == Tag::Multiply || Look().tag == Tag::Divide) {
		...
		if (t.tag == Tag::Multiply) {
			x = std::make_shared<BinaryExpression>(
				GetLoc(start), ExpressionType::Multiply, x, y);
		} else {
			x = std::make_shared<BinaryExpression>(
				GetLoc(start), ExpressionType::Divide, x, y);
		}
	}
	return x;
}
	...
	while (Look().tag == Tag::LeftParenthesis ||
		...
		if (Look().tag == Tag::LeftParenthesis) {
			...
			if (Look().tag == Tag::RightParenthesis) {
				...
				x = std::make_shared<InvocationExpression>(GetLoc(start), x,
														   arguments);
			} else {
