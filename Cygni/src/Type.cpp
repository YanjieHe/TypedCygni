...
bool Type::IsPrimitive(TypePtr type) {
  return type->typeCode == TypeCode::Int32 ||
         type->typeCode == TypeCode::Int64 ||
         type->typeCode == TypeCode::Float32 ||
         type->typeCode == TypeCode::Float64;
}
...
std::u32string StringType::ToString() const { return U"Predef.String"; }
...
FunctionType::FunctionType(TypePtr selfType, std::u32string name,
                           std::vector<TypePtr> parameters, TypePtr returnType)
    : Type(TypeCode::Function), selfType{selfType}, name{name},
...
ClassType::ClassType(PackageRoute route, std::u32string name)
    : Type(TypeCode::Class), route{route}, name{name} {}
...
TypePtr ClassType::Replace(
    const Table<std::u32string, TypeParameter> &parameters) const {
  if (parameters.ContainsKey(name)) {
    return std::make_shared<ClassType>(route,
                                       parameters.GetValueByKey(name).name);
  } else {
...
InterfaceType::InterfaceType(PackageRoute route, std::u32string name)
    : Type(TypeCode::Interface), route{route}, name{name} {}
...
TypeGraph::InheritanceChain(std::shared_ptr<ClassType> originalType) {
  std::stack<int> stack;
  std::unordered_set<int> visited;
...
TypeGraph::GetAllInterfaces(std::shared_ptr<ClassType> originalType) {
    ...
    while (!stack.empty()) {
      ...
      if (visited.find(node) == visited.end()) {
        ...
        if (types[node]->typeCode == TypeCode::Interface) {
          auto interfaceType =
              std::static_pointer_cast<InterfaceType>(types[node]);
...
std::vector<std::shared_ptr<InterfaceType>>
TypeGraph::GetAllSuperInterfaces(std::shared_ptr<InterfaceType> originalType) {
  std::vector<std::shared_ptr<InterfaceType>> interfaceList;
  std::stack<int> stack;
  std::unordered_set<int> visited;
  if (table.find(originalType) != table.end()) {
...
TypeGraph::GetAllSuperTypes(std::shared_ptr<Type> originalType) {
...
UnresolvedType::UnresolvedType(PackageRoute route, std::u32string name)
    : Type(TypeCode::Unresolved), route{route}, name{name} {}
