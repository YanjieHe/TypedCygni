...
#define READ_U16(VALUE) (VALUE) = (((uint16_t)code[pc]) << 8) + ((uint16_t)code[pc + 1]); pc = pc + 2;

#define BYTES_TO_U16(BYTES, OFFSET) ((((uint16_t)(BYTES)[(OFFSET)]) << 8) + ((uint16_t)(BYTES)[(OFFSET) + 1]))

#define STACK_PUSH_I32(VALUE) sp++; stack[sp].u.i32_v = (VALUE);
#define STACK_PUSH_I64(VALUE) sp++; stack[sp].u.i64_v = (VALUE);
#define STACK_PUSH_F32(VALUE) sp++; stack[sp].u.f32_v = (VALUE);
#define STACK_PUSH_F64(VALUE) sp++; stack[sp].u.f64_v = (VALUE);
#define STACK_PUSH_OBJECT(VALUE) sp++; stack[sp].u.gc_obj = (VALUE); stack[sp].is_gc_obj = true;
#define STACK_PUSH_FUNCTION(VALUE) sp++; stack[sp].u.function = (VALUE);

#define STACK_POP_I32(VALUE) (VALUE) = stack[sp].u.i32_v; sp--;
#define STACK_POP_I64(VALUE) (VALUE) = stack[sp].u.i64_v; sp--;
#define STACK_POP_F32(VALUE) (VALUE) = stack[sp].u.f32_v; sp--;
#define STACK_POP_F64(VALUE) (VALUE) = stack[sp].u.f64_v; sp--;
#define STACK_POP_OBJECT(VALUE) (VALUE) = stack[sp].u.gc_obj; stack[sp].is_gc_obj = false; sp--;
...
void run(Machine *machine)
{
	...
	constant_pool = machine->function->constant_pool;
	stack = machine->stack;
	pc = 0;
	fp = 0;
	...
	while (pc < cur_func->code_length)
	{
		...
		case PUSH_I32: {
			...
			READ_U16(index);
			const_i32 = constant_pool[index].i32_v;
		...
		case PUSH_I64: {
			...
			READ_U16(index);
			i64_v = constant_pool[index].i64_v;
		...
		case PUSH_F32: {
			...
			READ_U16(index);
			const_f32 = constant_pool[index].f32_v;
		...
		case PUSH_F64: {
			...
			READ_U16(index);
			const_f64 = constant_pool[index].f64_v;
		...
		case PUSH_STRING: {
			...
			READ_U16(index);
			const_str = constant_pool[index].str_v;
		...
		case PUSH_STATIC_I32: {
			...
			READ_U16(index);
			READ_U16(offset);
			static_i32 = machine->exe->classes[index].static_v[offset].u.i32_v;
		...
		case PUSH_STATIC_I64: {
			...
			READ_U16(index);
			READ_U16(offset);
			static_i64 = machine->exe->classes[index].static_v[offset].u.i64_v;
		...
		case PUSH_STATIC_F32: {
			...
			READ_U16(index);
			READ_U16(offset);
			static_f32 = machine->exe->classes[index].static_v[offset].u.f32_v;
		...
		case PUSH_STATIC_F64: {
			...
			READ_U16(index);
			READ_U16(offset);
			static_f64 = machine->exe->classes[index].static_v[offset].u.f64_v;
		...
		case PUSH_STATIC_OBJECT: {
			...
			READ_U16(index);
			READ_U16(offset);
			static_obj = machine->exe->classes[index].static_v[offset].u.gc_obj;
		...
		case POP_STATIC_I32: {
			...
			machine->exe->classes[index].static_v[offset].u.i32_v = static_i32;
			break;
		}
		case POP_STATIC_I64: {
			uint16_t offset;
			...
			machine->exe->classes[index].static_v[offset].u.i64_v = static_i64;
			break;
		}
		case POP_STATIC_F32: {
			uint16_t offset;
			...
			machine->exe->classes[index].static_v[offset].u.f32_v = static_f32;
			break;
		}
		case POP_STATIC_F64: {
			uint16_t offset;
			...
			machine->exe->classes[index].static_v[offset].u.f64_v = static_f64;
			break;
		}
		case POP_STATIC_OBJECT: {
			...
			machine->exe->classes[index].static_v[offset].u.gc_obj = static_obj;
			break;
		}
		case ADD_I32: {
		...
		case RETURN_I32: {
			...
			code = prev_func->code;
			constant_pool = prev_func->constant_pool;
			cur_func = prev_func;
		...
		case INVOKE: {
			...
			if (next_func->flag == METHOD_FLAG_NATIVE_FUNCTION)
			{
				...
				{
					native_function->function_pointer = load_library_function(
						machine->state, native_function->lib_name, native_function->entry_point);
			...
			else
			{
				...
				sp = stack_index + 3;
				code = next_func->code;
				constant_pool = next_func->constant_pool;
			}
			break;
		}
					 // case INVOKE: {
		...
		case PUSH_FUNCTION: {
			...
			READ_U16(module_index);
			READ_U16(function_offset);

			STACK_PUSH_FUNCTION(&(machine->exe->classes[module_index].methods[function_offset]));
			break;
		}
		//case PUSH_METHOD: {
		//	READ_USHORT(index);
		...
		//	sp++;
		//	stack[sp].u.function =
		//		machine->exe->classes[index].methods[offset]; 
		//	break;
		//}
		case NEW: {
		...
		case NEW_ARRAY: {
			...
			case TYPE_I32: {
				...
				new_array->arr->u.i32_array = (int32_t*)malloc(sizeof(int32_t) * length);
			...
			case TYPE_I64: {
				...
				new_array->arr = (Array *)malloc(sizeof(Array));
				new_array->arr->length = length;
				new_array->arr->u.i64_array = (int64_t*)malloc(sizeof(int64_t) * length);
			...
			case TYPE_F32: {
				...
				new_array->arr = (Array *)malloc(sizeof(Array));
				new_array->arr->length = length;
				new_array->arr->u.f32_array = (float_t*)malloc(sizeof(float_t) * length);
			...
			case TYPE_F64: {
				...
				new_array->arr = (Array *)malloc(sizeof(Array));
				new_array->arr->length = length;
				new_array->arr->u.f64_array = (double_t*)malloc(sizeof(double_t) * length);
			...
			case TYPE_OBJECT: {
				...
				new_array->arr = (Array *)malloc(sizeof(Array));
				new_array->arr->length = length;
				new_array->arr->u.obj_array = (GCObject*)malloc(sizeof(GCObject) * length);
...
void view_stack(Machine *machine, Value *stack, int sp)
{
	for (int i = 0; i <= sp; i++)
	{
		...
		if (stack[i].is_gc_obj)
		{
			...
			{
				printf("[OBJECT]  %s",
					machine->exe->classes[stack[i].u.gc_obj->obj->type_id].name);
			}
			printf("\n");
		}
		else
