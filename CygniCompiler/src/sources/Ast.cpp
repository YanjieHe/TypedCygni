...
IfThen::IfThen(Position position, Ptr<Ast> condition, Ptr<Ast> ifTrue)
    : Ast(Kind::IfThen, position), condition{std::move(condition)},
      ifTrue{std::move(ifTrue)} {}

IfElse::IfElse(Position position, Ptr<Ast> condition, Ptr<Ast> ifTrue,
               Ptr<Ast> ifFalse)
    : Ast(Kind::IfElse, position), condition{std::move(condition)},
...
Constant::Constant(Position position, Constant::ConstantType constantType,
    ...
    : Ast(Kind::Constant, position),
      constantType{constantType}, value{std::move(value)} {}
...
While::While(Position position, Ptr<Ast> condition, Ptr<Ast> body)
    : Ast(Kind::While, position), condition{std::move(condition)},
      body{std::move(body)} {}

Call::Call(Position position, Ptr<Ast> function, Vector<Ptr<Ast>> arguments)
    : Ast(Kind::Call, position), function{std::move(function)},
      arguments{std::move(arguments)} {}

Var::Var(Position position, String name, Optional<Ptr<TypeExpression>> type,
         Optional<Ptr<Ast>> value)
    : Ast(Kind::Var, position), name{std::move(name)}, type{std::move(type)},
      value{std::move(value)}, access{Access::Local} {}

Parameter::Parameter(String name, Ptr<TypeExpression> type)
    : name{std::move(name)}, type{std::move(type)} {}

Def::Def(Position position, String name, Vector<Parameter> parameters,
         Ptr<TypeExpression> type, Ptr<Ast> body)
    : Ast(Kind::Def, position), name{std::move(name)}, parameters{std::move(
                                                           parameters)},
      type{std::move(type)}, body{std::move(body)}, access{Access::Local} {}
...
DefClass::DefClass(Position position, String name, Vector<Ptr<Var>> fields,
                   Vector<Ptr<Def>> methods)
    : Ast(Kind::DefClass, position), name{std::move(name)},
      fields{std::move(fields)}, methods{std::move(methods)} {}

Program::Program(String path, Vector<Ptr<DefClass>> classes,
                 Vector<Ptr<DefModule>> modules)
    : path{std::move(path)}, classes{std::move(classes)}, modules{std::move(
                                                              modules)} {}
...
DefModule::DefModule(Position position, String name, Vector<Ptr<Var>> fields,
                     Vector<Ptr<Def>> methods)
    : Ast(Kind::DefModule, position), name{std::move(name)},
      fields{std::move(fields)}, methods{std::move(methods)} {}
