...
public:
  ...
  inline size_t size() const { return characters.size(); }

  inline std::basic_string<Char>::iterator begin() {
    return characters.begin();
  }

  inline std::basic_string<Char>::iterator end() { return characters.end(); }
  ...
  static bool ParseBool(const String &s) {
    if (s.size() == 4 && s[0] == 't' && s[1] == 'r' && s[2] == 'u' &&
        s[3] == 'e') {
      return true;
    } else if (s.size() == 5 && s[0] == 'f' && s[1] == 'a' && s[2] == 'l' &&
               s[3] == 's' && s[4] == 'e') {
      return false;
    } else {
      throw StringConversionException();
  ...
  inline static String ToString(int32_t value) { return std::to_string(value); }

  inline static String ToString(double_t value) {
    return std::to_string(value);
  }

  inline static String ToString(int64_t value) { return std::to_string(value); }

  inline static String ToString(bool value) { return value ? "true" : "false"; }
  ...
  inline void push_back(Char c) { characters.push_back(c); }

  inline void clear() { characters.clear(); }

  inline size_t find(Char c) { return characters.find(c); }
